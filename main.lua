...
Fwv = function (entity)
    ...
    if beltOn then
		...
		DisableControlAction(0, 185)
		DisableControlAction(0, 251)
	end
      ...
      if IsControlJustReleased(0, 305) and GetLastInputMethod(0) then
        beltOn = not beltOn 
        if beltOn then 
			exports['mythic_notify']:DoHudText('inform', 'Βάλατε την ζώνη σας.')
		  ...
		  isUiOpen = true 
			...
			exports['mythic_notify']:DoHudText('inform', 'Βγάλατε την ζώνη σας.')
